...
describe("render", () => {
  ...
  it("serves room client behavior from a separate script artifact", () => {
    ...
    expect(ROOM_CLIENT_SCRIPT).toContain("syncLegacyVoteHiddenFields(legacyVoteCommands[selectedTarget.value], latestGame);");
    ...
    expect(ROOM_CLIENT_SCRIPT).toContain('radio.addEventListener("change", () => syncLegacyVoteHiddenFields(action.command, game));');
    expect(ROOM_CLIENT_SCRIPT).toContain("radio.checked = true;");
    expect(ROOM_CLIENT_SCRIPT).toContain("syncLegacyVoteHiddenFields(action.command, game);");
