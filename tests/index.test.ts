...
describe("worker routes", () => {
  it("returns room options in room listings", async () => {
    const response = await worker.fetch(
      ...
      envWithRooms(["room_plain", "room_poison"], {}, {}, {}, {}, { room_poison: "poison wfbig authority decide" })
    );
      ...
      rooms: [
        {
          ...
          options: { poison: false, bigWolf: false, authority: false, decider: false }
        },
        {
          id: "room_poison",
          name: "poison",
  ...
  it("stores selected room options when creating rooms", async () => {
    ...
    expect(roomInsert?.values.at(-1)).toBe("poison wfbig authority decide");
