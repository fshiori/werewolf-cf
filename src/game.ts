...
export function castLobbyStartVote(
  ...
  const voter = state.players.find((player) => player.playerId === playerId);
  if (!voter) {
    throw new Error("Start vote player not found");
  }
  const alreadyVoted = state.lobbyStartVotes?.[playerId] === true;
...
function startGameWithPlayers(state: GameState, players: GamePlayer[], now: number, options: RoomOptions): GameState {
  return {
    ...
    lastWordsEnabled: options.lastWords,
