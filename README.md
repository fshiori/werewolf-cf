...
- `game_state`: room id, public phase, day, players, `openVote`/`selfVote`/`voteStatus` vote options, winner, timer, and log state.
...
- `action_ack`: confirms `vote`, `night_kill`, `divine`, `guard`, `child_fox_divine`, `cat_revive`, `kick_player`, `leave_room`, `room_end_vote`, or GM phase/end/life/role/flag/option updates.
