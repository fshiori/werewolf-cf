...
| Rendered non-JSON pages | `/`, `/list`, `/logs`, `/bbs`, `/admin/rooms`, `/room/:id`, `/leaderboard`, `/stats`, `/icons`, `/trips`, `/status`, `/room/:id/records`, `/room/:id/events`, `/player/:id`, `/rules`, `/script-info`, `/protocol`, `/version` | Implemented |
