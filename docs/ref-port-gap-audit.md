...
| Old logs (`old_log.php`) | `/room/:roomId/log` renders the D1 game-record and filtered room-event transcript summary; public chat plus private talk/vote/night-action replay now persist, with private entries visible only after the room ends | Partial |
...
1. Expand `/room/:roomId/log` replay formatting into reference-like per-day talk/vote/action tables; private talk, vote, and action transcript persistence with room-status visibility rules is now in place.
