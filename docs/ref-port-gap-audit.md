...
| Icon catalog/upload parity | `/icons` renders the reference default icon catalog with icon-view/upload title and background image chrome, local default-icon picker controls, and R2 avatar upload/remove controls; room join can send a vetted default `iconPath`; avatar upload exists on room and icon pages | Partial |
