...
| Icon catalog/upload parity | `/icons` renders the reference default icon catalog with local default-icon picker controls and R2 avatar upload/remove controls; room join can send a vetted default `iconPath`; avatar upload exists on room and icon pages | Partial |
