...
| Reference icons and bitmap assets | R2 upload exists; `docs/reference-asset-inventory.md` classifies 130 reference assets; `/assets/reference/:path` serves vetted copied R2 assets; top chrome, home/admin/federated room-list status/options, old-log title/background/capacity/discrete reference option/victory rows, icon catalog/upload title/background chrome, rules role rows, live/profile/transcript/room-record role markers, and game-record victory rows emit reference image URLs with text fallback | Partial |
...
| Old logs (`old_log.php`) | `/logs` and `/old_log.php` render a reference-style ended-room index with legacy `old_log.php?all=1`, `old_log.php?search=:term` search filtering, 25-row pagination with search-preserving page links, latest game-record victory icons, and `old_log.php?log_mode=on&room_no=:id` normal/reverse/heaven links plus max-player icons and the reference twelve fixed option columns; `/room/:roomId/records`, `/room/:roomId/log`, `/old_log.php?log_mode=on&room_no=:id`, and `/game_log.php?room_no=:id` render D1 game records with role-icon player result rows; room record/event/transcript headers link back through `game_view.php?room_no=:id`, and record/event headers link `game_log.php?room_no=:id&log_mode=on`; `/room/:roomId/log` and the PHP-style `old_log.php`/`game_log.php` aliases also render day/phase transcript sections, historical vote tables grouped by day/revote round with aggregate and per-ballot target counts, PHP-style `N 日目 ( M 回目)` vote headings, bordered zero-spacing vote table attributes, bold voter/target cells, reference-inspired vote-table coloring, location labels/colors for public/private/system/action rows, `heaven_talk`/`heaven_only`/`reverse_log` display modes, state-preserving `old_log.php`/`game_log.php?log_mode=on` header aliases, a reference-style old-log return link, and additive `viewer=public/player/dead/gm` masking modes with visible scope labels, player-view links/selectors, own private rows, addressed GM whispers, and saved-role-derived same-team wolf/fox/common/lovers channel rows; private entries are visible only after the room ends | Partial |
