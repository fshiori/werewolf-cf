...
| Reference icons and bitmap assets | R2 upload exists; `docs/reference-asset-inventory.md` classifies 130 reference assets; `/assets/reference/:path` serves vetted copied R2 assets; top chrome, home/admin/federated room-list status/options, old-log capacity/options, icon catalog/upload title/background chrome, rules role rows, live/profile/transcript/room-record role markers, and game-record victory rows emit reference image URLs with text fallback | Partial |
...
| Federated room list (`list.php`) | `/list` renders a reference-style federated list from local D1 rooms and optional remote peers configured by KV `federated_servers`, including reference status icons and peer health markers; configured peer status is shown, including failed peers; `/api.php` emits a reference-style tab-separated active-room feed for legacy federated consumers, and configured peers can fall back from JSON `/api/rooms` to legacy `/api.php` TSV feeds | Partial |
| Discussion board (`bbs.php`) | `/bbs` renders a reference-style topic list with pinned/locked/digest status markers, legacy `bbs.php?go=post`/`bbs.php?go=dige` subnavigation, digest-topic list linked from the side menu, paginated topic lists, topic titles linked to the latest reply page, topic detail views via `/bbs?view=:id`, legacy `/bbs.php?go=postre&id=:id`, legacy `/bbs.php?go=edit&id=:id`, and `/bbs/:id`, paginated replies, post form, reply form, locked-topic no-reply notice, and topic moderation panel backed by D1 `bbs_topics`/`bbs_replies`; topic/reply forms accept optional edit/delete passwords and store hashes in D1; topic and reply content can be edited by either BBS admin token or matching post password; topics and replies can be deleted by either BBS admin token or matching post password while maintaining reply counts; JSON topic creation returns the D1 topic id, and legacy `POST /bbs.php?go=post` accepts reference `bname`/`bpass`/`title`/`mess` fields before redirecting to `/bbs/:id`; the rendered topic list also exposes a reference-field legacy post form for the same handler; JSON reply creation returns the target page, and legacy `POST /bbs.php?go=postre` accepts reference `id`/`bname`/`bpass`/`mess` fields before redirecting to the paginated topic page; the rendered topic detail also exposes a reference-field legacy reply form for the same handler; legacy `POST /bbs.php?go=edit&id=:id` accepts reference `editis`/`password` form operations for topic edit/delete, reply edit/delete, and admin-only topic lock/pin/digest toggles; `/admin/bbs` lists paginated topics with the same status markers and links to token-protected moderation controls that can pin, lock, mark digest, edit topic/reply content, delete topics with their replies, and delete individual replies while maintaining reply counts | Partial |
| Icon catalog/upload parity | `/icons` renders the reference default icon catalog with icon-view/upload title and background image chrome, local default-icon picker controls, and R2 avatar upload/remove controls; room join can send a vetted default `iconPath`; avatar upload exists on room and icon pages; legacy `upload.php` and `upload2.php` POST aliases reuse the same validated R2 avatar handlers and accept reference-style form field names | Partial |
| Old logs (`old_log.php`) | `/logs` and `/old_log.php` render a reference-style ended-room index with legacy `old_log.php?log_mode=on&room_no=:id` normal/reverse/heaven links plus max-player/option icons; `/room/:roomId/records`, `/room/:roomId/log`, `/old_log.php?log_mode=on&room_no=:id`, and `/game_log.php?room_no=:id` render D1 game records with role-icon player result rows; `/room/:roomId/log` and the PHP-style `old_log.php`/`game_log.php` aliases also render day/phase transcript sections, historical vote tables grouped by day/revote round with aggregate and per-ballot target counts, reference-inspired vote-table coloring, location labels/colors for public/private/system/action rows, `heaven_talk`/`heaven_only`/`reverse_log` display modes, and additive `viewer=public/player/dead/gm` masking modes with visible scope labels, player-view links/selectors, own private rows, addressed GM whispers, and saved-role-derived same-team wolf/fox/common/lovers channel rows; private entries are visible only after the room ends | Partial |
| Trip identity parity | `/trip` provides reference-style identity registration, claim, exclusion, and lookup controls with status markers, legacy subnavigation, plus legacy form actions for Trip registration and record exclusion; `/trips` and `/api/trips/lookup` provide public lookup with registered/excluded status markers, claimed players, and aggregate stats without exposing Trip hashes; legacy `trip.php?go=trip&id=:trip` renders a server-side public Trip detail table without exposing stored hashes; legacy `trip.php?go=room&id=:trip` renders server-side participation records from claimed player game records without exposing stored hashes; legacy `trip.php?go=icon` opens the shared reference-style icon catalog/upload flow; legacy `POST /trip.php?go=post` accepts reference `name`/`password` fields for Trip registration and legacy `POST /trip.php?go=out` accepts reference `name`/`password`/`aname` fields for Trip exclusion before redirecting to `/trip` | Partial |
