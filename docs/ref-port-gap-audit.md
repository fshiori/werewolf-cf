...
| Discussion board (`bbs.php`) | `/bbs` renders a reference-style topic list with pinned/locked/digest status markers, digest-topic list linked from the side menu, topic detail view, post form, reply form, and topic moderation panel backed by D1 `bbs_topics`/`bbs_replies`; `/admin/bbs` lists topics with the same status markers and links to token-protected moderation controls that can pin, lock, and mark digest topics | Partial |
