...
| Discussion board (`bbs.php`) | `/bbs` renders a reference-style topic list with pinned/locked/digest status markers, digest-topic list linked from the side menu, topic detail views via `/bbs?view=:id` and `/bbs/:id`, post form, reply form, and topic moderation panel backed by D1 `bbs_topics`/`bbs_replies`; new topics return their D1 topic id and the form redirects to `/bbs/:id`; `/admin/bbs` lists topics with the same status markers and links to token-protected moderation controls that can pin, lock, mark digest, and delete topics with their replies | Partial |
