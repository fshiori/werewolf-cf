...
| Old logs (`old_log.php`) | `/logs` and `/old_log.php` render a reference-style ended-room index with legacy `old_log.php?all=1` and `old_log.php?log_mode=on&room_no=:id` normal/reverse/heaven links plus max-player/option icons; `/room/:roomId/records`, `/room/:roomId/log`, `/old_log.php?log_mode=on&room_no=:id`, and `/game_log.php?room_no=:id` render D1 game records with role-icon player result rows; `/room/:roomId/log` and the PHP-style `old_log.php`/`game_log.php` aliases also render day/phase transcript sections, historical vote tables grouped by day/revote round with aggregate and per-ballot target counts, reference-inspired vote-table coloring, location labels/colors for public/private/system/action rows, `heaven_talk`/`heaven_only`/`reverse_log` display modes, and additive `viewer=public/player/dead/gm` masking modes with visible scope labels, player-view links/selectors, own private rows, addressed GM whispers, and saved-role-derived same-team wolf/fox/common/lovers channel rows; private entries are visible only after the room ends | Partial |
