...
| Browser screenshot capture exists for static/lobby/frame, core game-state UI, and old-log viewer modes | `scripts/capture-visual-parity.mjs`; captures home/list/icons/trip/BBS/stats/status plus temporary room lobby/spectator, `game_frame.php`/`game_up.php`/bottom/`game_vote.php`, and with `--include-game-states` day/night/ended/public/player/dead/GM old-log pages across desktop/tablet/mobile; fails on broken `<img>` assets | Partial browser evidence |
...
| Full screenshot parity against the PHP reference | `docs/visual-parity-checklist.md` exists and Playwright Chromium capture now covers static/lobby/frame/day/night/ended and public/player/dead/GM old-log pages with broken-image checks, but manual comparison against the PHP reference and GM live-control screenshots remain. |
