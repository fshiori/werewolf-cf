...
| GM controls and whispers | `game_play.php`, `game_vote.php` `GMVote` / `GM_CHANNEL`, admin/GM paths | `src/room.ts`, `src/game.ts` force controls, common-voice toggle, and `chdis` channel restriction persistence; `src/room-client.ts` GM controls | `tests/game.test.ts` force-end/role/life/flag/channel restriction tests; `tests/room.test.ts` GM websocket, whisper, common-voice, and channel-restriction tests | Partial | Core controls, common voice, and the legacy-style `chdis:ch_wolf:ch_common:ch_lovers:ch_fox` restriction matrix exist; remaining admin screens and exact PHP self-talk rerouting presentation are still partial. |
