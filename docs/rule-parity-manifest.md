...
| --- | --- | --- | --- | --- | --- |
| Role deck thresholds | `game_functions.php`, `room_manager.php` | `src/game.ts` `REFERENCE_ROLE_DECKS`, `startGame` | `tests/game.test.ts` role deck tests for 3/5/6/7/8/13/15 players | Implemented | More large-size deck snapshots can be added if a line-by-line role count table is extracted. |
| Optional roles: poison, big wolf, authority, decider, lovers, betrayer, child fox, two foxes, cat | `room_manager.php`, `game_functions.php` | `src/game.ts` `applyRoomOptions` | `tests/game.test.ts` option tests for poison, big wolf, authority/decider, lovers, betrayer, child fox, two foxes, cat | Implemented | Cult is separate and remains missing. |
| Wish role | `user_manager.php`, `game_functions.php` | `src/game.ts` wish-role handling in `startGame` | `tests/game.test.ts` "honors available wished roles..." | Partial | Basic available-role preference exists; PHP-era exact shuffle/priority behavior still needs deeper extraction. |
...
| Day vote, revote, authority, decider, self-vote | `game_play.php`, `game_functions.php` | `src/game.ts` `castDayVote`, `resolveDay`, vote target helpers | `tests/game.test.ts` authority, decider, self-vote, day-to-night, revote tests; `tests/room.test.ts` vote validation tests | Implemented | Active-room spectator/dead vote display is still a UI parity gap. |
| Open vote and vote status visibility | `game_functions.php` vote output | `src/messages.ts`, `src/room-client.ts`, `src/render.ts` | `tests/messages.test.ts` vote mappings/status tests; `tests/render.test.ts` voted player styling tests | Partial | Public message privacy is covered; full PHP vote table variants remain partial. |
...
| GM controls and whispers | `game_play.php`, admin/GM paths | `src/room.ts`, `src/game.ts` force controls | `tests/game.test.ts` force-end/role/life/flag tests; `tests/room.test.ts` GM websocket and whisper tests | Partial | Core controls exist; legacy admin room-management screens are separate. |
| Timed phases, silence, sudden death | `game_play.php` flow checks | `src/game.ts` alarms, silence acceleration, warning/death handling | `tests/game.test.ts` timer defaults, silence acceleration, realtime no-acceleration, sudden-death warnings; `tests/room.test.ts` DO alarm tests | Implemented | UI sound for revote/daybreak warning remains partial. |
| Lobby start, leave, kick votes | `user_manager.php`, `room_manager.php` resident handling | `src/game.ts` lobby vote/leave/kick helpers; `src/room.ts` websocket commands | `tests/game.test.ts` lobby host/start/kick/leave tests; `tests/room.test.ts` websocket lobby start/kick/leave tests | Partial | Current implementation covers resident start/kick/leave; any PHP reset-vote nuance still needs deeper mapping. |
...
| Cult role/option | `game_functions.php` assets/talk-role references | None | None | Missing | Current `PlayerRole` has no cult role. Requires rule extraction before implementation. |
| Viewer-specific transcript masking | `game_functions.php` talk output branches | `src/index.ts` private-event filtering; `src/render.ts` transcript grouping/styling | `tests/index.test.ts` private event hidden until ended; `tests/render.test.ts` transcript location styling | Partial | Current old log hides private rows before room end and styles locations after end; per-viewer alive/dead/GM masking is not fully PHP-compatible. |
...
1. Extract the remaining PHP cult-role behavior before adding any `PlayerRole` value.
2. Add focused tests for any newly discovered victory edge cases from `game_functions.php`.
3. Decide whether viewer-specific old-log masking should be implemented as separate endpoints/modes or as parameters on `/room/:roomId/log`.
4. Add screenshot-based UI parity once a browser is available.
