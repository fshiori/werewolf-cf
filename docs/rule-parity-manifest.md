...
| Day vote, revote, authority, decider, self-vote | `game_play.php`, `game_functions.php` | `src/game.ts` `castDayVote`, `resolveDay`, vote target helpers; `src/room-client.ts` live observer/dead vote panel; `src/render.ts` old-log vote round totals | `tests/game.test.ts` authority, decider, self-vote, day-to-night, revote tests; `tests/room.test.ts` vote validation tests; `tests/render.test.ts` observer/dead vote panel and transcript vote-total artifact checks | Implemented | Exact PHP vote table layout variants remain a visual parity gap. |
| Open vote and vote status visibility | `game_functions.php` vote output | `src/messages.ts`, `src/room-client.ts`, `src/render.ts` | `tests/messages.test.ts` vote mappings/status tests; `tests/render.test.ts` voted player styling, observer/dead vote panel, and old-log vote total tests | Partial | Public message privacy, live observer/dead status, and historical per-target vote totals are covered; exact PHP vote table variants remain partial. |
...
| GM controls and whispers | `game_play.php`, `game_vote.php` `GMVote` / `GM_CHANNEL`, admin/GM paths | `src/room.ts`, `src/game.ts` force controls, common-voice toggle, and `chdis` channel restriction persistence; `src/index.ts` room-summary parsing; `src/render.ts` admin room context; `src/room-client.ts` GM controls | `tests/game.test.ts` force-end/role/life/flag/channel restriction tests; `tests/room.test.ts` GM websocket, whisper, common-voice, and channel-restriction tests; `tests/index.test.ts` room option/admin parsing; `tests/render.test.ts` room-list/admin markers | Partial | Core controls, common voice, room summaries, admin waste-room context, and the legacy-style `chdis:ch_wolf:ch_common:ch_lovers:ch_fox` restriction matrix exist; broader admin screens and exact PHP self-talk rerouting presentation are still partial. |
...
| Viewer-specific transcript masking | `game_functions.php` talk output branches | `src/index.ts` private-event filtering and explicit transcript viewer query validation; `src/render.ts` transcript grouping/styling plus `viewer=public/player/dead/gm` masks and GM channel-control system rows | `tests/index.test.ts` private event hidden until ended and transcript viewer query tests, including required `viewer_player_id`; `tests/render.test.ts` transcript location styling, viewer-mode filtering, and heaven-only GM operation visibility | Partial | Additive viewer modes now cover public, own-player private, dead/heaven, and GM transcript perspectives; player mode requires an explicit viewer id, but exact PHP layout/authenticated identity semantics remain partial. |
