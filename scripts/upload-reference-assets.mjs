...
function wranglerArgs(bucket, key, file, config) {
...
export function plannedReferenceAssetUploads({
  ...
  return PRIORITY_ASSETS.map((assetPath) => {
    ...
    return {
      ...
      file,
      key,
      command: ["npx", ...wranglerArgs(bucket, key, file, config)]
    };
  });
}
...
if (isMain) {
  ...
  const plan = plannedReferenceAssetUploads({ refRoot, bucket, prefix, config });
